...
function Type_B() {
  ...
  let numberOfColumns = typeB_content.length;
  const [TypeBContent, setTypeBContent] = useState(typeB_content);
  const [editIndex, setEditIndex] = useState(null);
  const [editContent, setEditContent] = useState('');

  function activateOption(index, option, type) {
    setEditIndex(index);
    setEditContent(TypeBContent[index].content);
    const updateTypeB_Content=[...TypeBContent];
    updateTypeB_Content[index].type=type;
    setTypeBContent(updateTypeB_Content)
  }

  function handleEdit(index, type) {
    setEditIndex(index);
    setEditContent(typeB_content[index].content);
  }

  function saveEdit() {
    if (editIndex !== null) {
      const updatedTypeBContent = [...TypeBContent];
      updatedTypeBContent[editIndex].content = editContent;
      setTypeBContent(updatedTypeBContent);
      setEditIndex(null);
      setEditContent('');
    }
  }
  ...
  function cancelEdit() {
    setEditIndex(null);
    setEditContent(typeB_content[editIndex].content);
  }

  return (
    <article>
      <div style={{ border: '1px solid gray' }}>
        <h2>Type_B Survey ADMIN</h2>
      </div>
      <div style={{ border: '1px solid gray' }}>
        <Grid2 container>
